...
// myErrorHandler 代理服务器的错误处理，打印日志+返回502
func myErrorHandler(rw http.ResponseWriter, req *http.Request, err error) {
    Logger.Error("http proxy error, now will return 502 status code", zap.Error(err), 
        zap.String("request info", getRequestString(req)),
        zap.String("fjk-trace-id", req.Header.Get("fjk-trace-id")),
    )
